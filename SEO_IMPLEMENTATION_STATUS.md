...
## 0. Current reconciled state (2026-09-19)
...
| **Recommendation Generation — Stage 1 (guarded generation RPC, backend only)** | **`IMPLEMENTED — LOCALLY VERIFIED — ACCEPTED — MODULE-LOCKED (2026-07-24)`.** Additive migration `20260724130000_seo_recommendation_generate.sql` adds two nullable columns (`source_issue_fingerprint`, `generation_method`) and two partial unique indexes (`WHERE is_current`) to `seo_recommendations`, plus one guarded `SECURITY DEFINER` RPC `public.seo_recommendation_generate(p_website_id uuid) RETURNS SETOF seo_recommendations` (`SET search_path=public`; `authenticated` EXECUTE, **anon + PUBLIC revoked up-front**). Converts real, crawler-detected `seo_audit_issues` (status `open`/`in_review` only, from the latest completed audit run) plus 7 fixed on-page templates into canonical `seo_recommendations` rows, reproducing the existing mock's `CATEGORY_TO_AREA` / `ACTION_TYPE_BY_FIX_OWNER` mapping and `ON_PAGE_TEMPLATES` server-side — no AI/LLM, no new categories. Authorizes owner/admin/team_member or global admin (client/anon/non-member/cross-tenant denied, one non-leaking message; missing website == unauthorized). **First real consumer of the existing `is_current`/`superseded_by` versioning**: three-way replace-to-match (insert-new / no-write-if-unchanged / supersede-if-changed-and-still-`suggested`-or-`needs_review` / leave-alone-if-a-human-already-acted / retire-if-resolved-and-untouched) — a human decision (`approved`, `ready_to_publish`, etc.) is never silently overwritten by a later regeneration. Returns the **canonical current recommendation set** (not a transient count).
  ...
  **GENUINE LOCAL VERIFICATION NOW COMPLETE (2026-07-24, later same day).** Following operator installation of Docker, a real local Supabase stack was started (`project_id = wt-recommendation-stage1`; isolation proven — private Docker-bridge address, container names distinct from any project ref, `.env.local`'s `VITE_SUPABASE_URL` directly confirmed to point at `Digi_SEO_Test`'s ref, categorically different). The deferred SSO migration's first-boot auto-apply conflict was identified and resolved via a proven, fully-reversible local-only mechanism (temporarily excluding the file during `db reset`, restoring it after — full method in `SEO_LOCAL_DATABASE_SETUP.md`), producing a baseline correctly consistent with `Digi_SEO_Test`'s deferred state, confirmed reproducible across two consecutive resets. The RPC contract was re-verified directly on the local database (owner/SECURITY DEFINER/search_path/grants/return-type all match). **The full SQL verification suite passed twice** (exit 0 both times, every NOTICE checkpoint — including `TEARDOWN ok — net-nothing` and `ALL RECOMMENDATION GENERATION STAGE 1 CHECKS PASSED` — printed and confirmed, independently re-confirmed with 0 residue). **The live two-session concurrency proof passed against the local database**: Session B directly observed `wait_event_type=Lock, wait_event=advisory` at two poll points while Session A held the lock via `pg_sleep(10)`; Session B's completion timing matched A's lock release almost exactly; post-race state was 8 current rows / 8 distinct identities / 0 duplicates. Full evidence: `SEO_RECOMMENDATION_GENERATION_STAGE1_VERIFICATION.md` §6.

  **Implementation state:** migration, RPC, SQL verification suite, rollback script, and this record were implemented and locally verified in a temporary worktree/branch (`feat/seo-recommendation-generate-stage1`, based on `origin/main` `71ac8fd0fd6087bb5435bea4cca865025bc27967`) before being committed (see "Acceptance + lock" below for the final, pushed/merged state). No frontend file, no locked module, no crawler table, and no approval-lifecycle code was touched. `Digi_SEO_Test` remains rolled back and untouched; production untouched.

  **Acceptance + lock + push/merge (2026-07-24):** ~~(1) a genuinely local PostgreSQL/Supabase environment must be made available~~ — **DONE**; ~~(2) genuine local Stage 1 database verification must be performed and pass~~ — **DONE**; ~~(3) Stage 1 must be reviewed and accepted~~ — **DONE, Stage 1 is formally ACCEPTED.** Committed as `808d54d457ad9be713440ce2513bd65d6a0f11ea` (`feat(seo): add guarded recommendation generation`) and `e7b1fbebfc9d99fd69bbaceb93d277b4bea36c42` (`docs(seo): lock recommendation generation stage 1`) on `feat/seo-recommendation-generate-stage1`, **pushed to `origin`, fast-forwarded onto `main`, and pushed as `origin/main`** (`71ac8fd..e7b1fbe`, no merge commit). **Recommendation Generation Stage 1 (backend only) is formally MODULE-LOCKED** — see `docs/markdown/MODULE_LOCKS.md` (new entry). This lock is narrower than every other locked module in the registry: **no frontend integration, no unit tests, and no authenticated operator/browser acceptance exist for this feature** — Stage 2 (frontend wiring, role gate, unit tests, operator acceptance) was deferred at the time of this lock and has **since been completed, locked and merged to `main`** — see the Stage 2 row below. **No further `Digi_SEO_Test` use is permitted for this feature without an approval explicitly recorded in the controlling ChatGPT instruction trail.** | **LOCKED** (Recommendation Generation Stage 1 backend-only approved scope, committed + pushed + merged to `main`; Stage 2 frontend is separately locked — next row) | `supabase/migrations/20260724130000_seo_recommendation_generate.sql`, `supabase/test/seo_recommendation_generate_verification.sql`, `supabase/test/seo_recommendation_generate_rollback_TEST_ONLY.sql`, `SEO_RECOMMENDATION_GENERATION_STAGE1_VERIFICATION.md`, `SEO_RECOMMENDATION_GENERATION_ARCHITECTURE.md` |
| **Recommendation Generation — Stage 2 (frontend integration)** | **`ACCEPTED — MODULE-LOCKED — MERGED TO CANONICAL main (accepted + locked 2026-07-24; fast-forwarded into main at 9cb3676 on 2026-09-19)`.** Wires the locked Stage 1 RPC into the real application on `WebsiteAuditPage.tsx`. New `generateSupabaseRecommendations(websiteId)` in `seoRecommendationSupabaseService.ts` calls `seo_recommendation_generate` (only `p_website_id`), validates the response is an array, then re-reads the canonical current set through the existing read path (`fetchSupabaseRecommendations`) — the backend's mapping logic is never reproduced client-side. `recommendationService.generateRecommendations` dispatches via `runWithServiceAdapter` (`fallbackToMockOnError:false`); the pre-existing mock generator is reused unchanged (self-deriving issues from the latest completed mock audit). **Role gating (usability layer only; the RPC remains authoritative):** `RECOMMENDATION_GENERATE_ROLES=['owner','admin','team_member']` + `canGenerateRecommendations(role, supabaseMode)`, mirroring the Competitor Stage 2B pattern exactly — real role via `getCurrentSeoRole` in Supabase mode, always-enabled in mock mode. New `RecommendationGenerationPanel.tsx`: current-count badge, link to Approval Queue, role-gated Generate/Refresh button (disabled + tooltip + `aria-disabled` for denied roles), loading state, `aria-live` error message, no-eligible-findings note, and copy stating generation does not auto-publish. `ApprovalQueuePage.tsx` / `approvalService.ts` / `seoApprovalSupabaseService.ts` were **not modified** — their existing idempotent `ensureApprovalQueueGenerated` pipeline already consumes real rows once they exist. **Unit tests (new, 15 total):** `seoRecommendationSupabaseService.test.ts` (+6 — exact RPC args, error-no-fallback, response-type validation, UUID validation, read-back-not-raw-payload proof, empty-array-is-valid) and `recommendationService.test.ts` (+9 — full role matrix, Supabase-branch dispatch args, error propagation unmasked, mock-branch still functions). Full suite **48/48 pass** (was 33/33); `tsc` clean; `npm run build` clean. **Local database verification (genuine local Docker-based Supabase stack, not `Digi_SEO_Test`, not production):** discovered and fixed one further local-environment-only gap beyond Stage 1's — `authenticated`/`anon` had no base table-level GRANTs (RLS was correctly enabled and defined on 100% of tables, but PostgREST denies before RLS is ever evaluated without the underlying grant); fixed via the standard Supabase default-privilege bootstrap applied directly to the local database only, RLS-enablement re-confirmed unchanged afterward. Real fixtures (owner/admin/team_member/client + a real completed audit with 2 issues) proved: generation persists real rows; UI refreshes without reload; repeat generation is idempotent (0 duplicates, DB-confirmed); an operator-approved recommendation survives a later regeneration untouched; client is denied both in the UI (disabled control) and at the backend (a direct in-page bypass fetch using the client's real session token returned the RPC's own non-leaking `P0001` denial — the backend remains the sole authoritative check). 0 fixture residue after teardown. **Browser acceptance performed live** (owner + client role walkthroughs, real authenticated sign-in, real accessibility-tree/DOM verification — no manual-check claims). **No locked Stage 1 file modified; no defect found in Stage 1.** **Acceptance-gap closure (2026-07-24, follow-up session):** the privilege-bootstrap gap was re-derived from a genuinely clean `supabase db reset` (not a previously-patched database) — before the fix, a raw anon-key REST request returned `401`/`42501`/`"permission denied for table seo_recommendations"` with PostgREST's own hint naming the exact missing `GRANT`; classified as a local-Supabase-CLI-only reproducibility gap (no repository migration is missing anything; `Digi_SEO_Test`/hosted Supabase provisions this automatically outside of migrations) — **no change to the locked Stage 1 migration/RPC was needed or made.** The fix is now a committed-ready, idempotent, RLS-preserving script (`supabase/test/local_supabase_privilege_bootstrap.sql`) with its own verification block, and the setup procedure is documented end-to-end in a new `SEO_LOCAL_DATABASE_SETUP.md`. The loading state (button `disabled=true`, text `"Generating..."`, exactly 1 RPC call, a second click while blocked producing 0 additional calls) and the no-eligible-findings state (a disposable second website with a completed audit and 0 issues correctly generating only the 7 on-page templates, 0 issue-derived rows, 0 auto-created approval items, confirmed idempotent on a second click) were both proven live against the real UI, not code-reviewed. All fixtures removed via a final `db reset` + bootstrap re-application, leaving the local database at 0 rows across every public table with grants and RLS intact. Full evidence: `SEO_RECOMMENDATION_GENERATION_STAGE2_VERIFICATION.md`. **Formal acceptance review complete (2026-07-24, same day) — no material defect found.** Committed as `36d32af2a3841267d19a7911ad7e693e6e10f81d` (`feat(seo): integrate recommendation generation workflow`) on `feat/seo-recommendation-generate-stage2` (based on `origin/main` `c1de7fe5400d88189d1b826d884ef31779ab2290`, same base as the locked Stage 1 backend). **Recommendation Generation Stage 2 (frontend integration) is now formally MODULE-LOCKED (2026-07-24)** — see `docs/markdown/MODULE_LOCKS.md` (new "Recommendation Generation — Stage 2 frontend integration" entry, added in a second commit alongside this document). The Stage 1 lock's scope (backend only) is unchanged by this work — Stage 1's own lock entry was not edited. **Historical (2026-07-24): committed locally only. Superseded: pushed and fast-forwarded into canonical `main` (`9cb3676`) on 2026-09-19.** | **LOCKED** (Recommendation Generation Stage 2 frontend-integration approved scope; merged to canonical `main`; Stage 1 backend lock remains separate/unchanged) | `src/services/supabase/seoRecommendationSupabaseService.ts`, `src/services/recommendationService.ts`, `src/pages/seo/WebsiteAuditPage.tsx`, `src/pages/seo/audit/RecommendationGenerationPanel.tsx`, `src/services/supabase/supabaseTypes.ts`, `src/services/recommendationService.test.ts`, `src/services/supabase/seoRecommendationSupabaseService.test.ts`, `SEO_RECOMMENDATION_GENERATION_STAGE2_VERIFICATION.md`, `SEO_LOCAL_DATABASE_SETUP.md`, `supabase/test/local_supabase_privilege_bootstrap.sql` |
| **Competitor Benchmarking — Stage 2B (frontend generation integration)** | **FRONTEND-IMPLEMENTED + UNIT-TESTED + AUTHENTICATED OPERATOR-ACCEPTED + MODULE-LOCKED on `Digi_SEO_Test` (2026-07-24).** Wires the Stage 2A RPC through the established adapter pattern: new `generateSupabaseCompetitors(websiteId)` in `seoCompetitorSupabaseService.ts` calls `supabase.rpc('seo_competitor_generate', { p_website_id })` (only the website id — no workspace/actor/scores/provenance/timestamps sent), validates the response is a numeric count, then re-reads the canonical set through the Stage 1 read path (`fetchSupabaseCompetitors`) — the heuristic is never reproduced client-side. `competitorService.generateCompetitorBenchmarkData` dispatches via `runWithServiceAdapter` (`fallbackToMockOnError: false`); the mock branch is the pre-existing local generation, extracted verbatim into `generateMockCompetitorBenchmarkData` (unchanged behaviour). **Role gating (usability layer only; the RPC remains authoritative):** new `canGenerateCompetitorBenchmarks(role, supabaseMode)` + `COMPETITOR_GENERATE_ROLES = ['owner','admin','team_member']` (mirrors `AuthorityBuilderPage`'s `CAMPAIGN_SUBMIT_ROLES` pattern) — `CompetitorAnalysisPage` queries the real `seo_workspace_members.seo_role` via the existing `getCurrentSeoRole` helper (Supabase mode only; mock mode stays always-enabled, unchanged) and disables Generate/Refresh with the established "Requires the owner, admin, or team member role." tooltip (same wording as the Stage 6 role-gated controls) for client/non-member/no-membership. Added an actionable error message on generation failure (mirrors `ReportsPage`'s pattern); empty-onboarding behaviour is a backend safety net (Stage 2A returns 0, non-destructive) since the UI already gates the Generate button behind a non-empty onboarding competitor list. **No heuristic scoring logic added to the frontend; no locked module touched; no backend authorization weakened.** **Unit tests (new, 16 total):** `seoCompetitorSupabaseService.test.ts` (+7 — exact RPC name/args incl. proof only `p_website_id` is sent; error propagates with no fallback attempt; RPC-response-type validation; successful generation reads back the persisted rows, not the RPC's raw payload; truthful `estimated` provenance survives the read-back) and new `competitorService.test.ts` (+9 — `canGenerateCompetitorBenchmarks` for owner/admin/team_member/client/null across mock and Supabase mode; Supabase-branch dispatch sends only the website id; a Supabase generation error propagates unmasked; mock-mode branch never calls the RPC and still functions against the seeded onboarding fixture). Full suite **33/33 pass** (was 20/20); `tsc` clean; `npm run build` clean. **Browser verification performed (mock mode, no auth needed):** `/seo/competitor-analysis` renders with Refresh enabled, a live Refresh click re-generates and reloads correctly (count/score/timestamp update, button returns to idle), no console errors beyond the pre-existing app-wide React-Router v7 future-flag warnings. **Browser verification performed (Supabase mode, unauthenticated, via the same temporary/since-restored `public/runtime-config.js` override used for Stage 1 — restored byte-for-byte, hash-verified, 0 residue):** unauthenticated access to `/seo/competitor-analysis` correctly redirects to the real Digibility sign-in page (`ProtectedRoute` route protection intact) — no data leak, no mock fallback, no console errors.
...
**COMMITTED + PUSHED + MERGED to `main` (2026-07-24, HEAD `a594d1d`).** Formal Competitor Benchmarking lock added to `docs/markdown/MODULE_LOCKS.md` (2026-07-24) covering Stage 1 + Stage 2A + Stage 2B; future changes require the documented unlock/additive-extension procedure. | **LOCKED** (part of **Competitor Benchmarking**, Stages 1–2; 2026-07-24) | `src/services/supabase/seoCompetitorSupabaseService.ts`, `src/services/competitorService.ts`, `src/pages/seo/CompetitorAnalysisPage.tsx`, `src/services/supabase/supabaseTypes.ts`, `src/services/supabase/seoCompetitorSupabaseService.test.ts`, `src/services/competitorService.test.ts` |
...
- **No SEO production Supabase project exists or has been identified. No
  production rollout has occurred.** No migration, RPC, worker, or config has been
  applied to any production project; Cloud Run has **not** been deployed. Hard
  invariant. `SEO_PRODUCTION_PROMOTION_PLAN.md` is a future planning reference,
  not a statement of readiness.
...
- **Competitor Benchmarking (Stages 1–2) — DONE, not a deferred item.**
  COMPLETE + LOCKED (2026-07-24; commits `2d5ff89`/`a594d1d`, fast-forwarded to
  `main`, pushed). See §1 (Competitor rows, now **LOCKED**) for the per-stage
  status and `docs/markdown/MODULE_LOCKS.md` (Competitor Benchmarking entry)
  for the protected contracts. Migration range `20260720123000`–`20260724120040`;
  full SQL/authz/determinism/replace-to-match verification + true two-session
  advisory-lock concurrency + authenticated operator acceptance (owner/admin/
  team_member/client) all PASS on `Digi_SEO_Test`. **Deferred/UNLOCKED (out of
  scope, not defects):** real external competitor-data provider integration,
  scheduled/automatic regeneration, competitor-count/plan-tier limits, CSV
  export, historical trend tracking.
- **Recommendation Generation — Stage 1 (backend only) — DONE, ACCEPTED,
  MODULE-LOCKED, MERGED TO `main` (2026-07-24), not a deferred item.**
  Genuinely locally verified (real Docker-based local Supabase stack; not
  `Digi_SEO_Test`, not production); pushed and fast-forwarded onto `main`
  (`71ac8fd..e7b1fbe`). See §1 (Recommendation Generation Stage 1 row) and
  `docs/markdown/MODULE_LOCKS.md` for full evidence and the protected-contract
  list.
- **Recommendation Generation — Stage 2 (frontend integration) — DONE,
  ACCEPTED, MODULE-LOCKED (2026-07-24), not a deferred item.** Frontend
  service wiring, role-gated "Generate Recommendations" UI control, 15 new
  unit tests, and genuine local-database + authenticated-browser
  verification are complete — see §1 (Recommendation Generation Stage 2
  row), `SEO_RECOMMENDATION_GENERATION_STAGE2_VERIFICATION.md`, and
  `docs/markdown/MODULE_LOCKS.md` (new entry) for full evidence and the
  protected-contract list. Committed as `36d32af`
  (`feat(seo): integrate recommendation generation workflow`) and `9cb3676`
  (lock documentation) — **merged into canonical `main` at `9cb3676` on
  2026-09-19** (fast-forward from `c1de7fe`; earlier "committed locally only"
  wording was accurate on 2026-07-24 and is superseded).
...
- **Competitor Benchmarking (Stages 1–2) — DONE, not a pending item.**
  COMPLETE + MODULE-LOCKED (2026-07-24; commits `2d5ff89`/`a594d1d`,
  fast-forwarded to `main` at HEAD `a594d1d`, pushed). Backend (Stage 2A)
  TEST-applied + concurrency-verified; frontend (Stage 2B) implemented +
  unit-tested + authenticated-operator-accepted for all four roles
  (owner/admin/team_member generation PASS; client denied in UI + backend);
  error path + responsive/regression checks PASS. See §1 (Competitor rows) and
  `docs/markdown/MODULE_LOCKS.md` (Competitor Benchmarking entry) for full
  evidence and the protected-contract list.
- **Recommendation Generation Stage 2 (frontend integration) — DONE, ACCEPTED,
  MODULE-LOCKED, MERGED TO CANONICAL `main` (`9cb3676`, 2026-09-19), not a
  pending item.** Both evidence gaps flagged during implementation were closed on
  2026-07-24 and the formal acceptance review found no material defect. **No Git
