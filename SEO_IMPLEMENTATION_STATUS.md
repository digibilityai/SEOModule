...
- **Reports backend wiring** — **Stage 1 (real-data READ foundation) is
  ...
  **no `localStorage` mock fallback** in Supabase mode; Generate **disabled** with
  ...
  TEST residue. **The Reports v1 lock is blocked solely on the Stage 2 operator
  two-session concurrency run** (no held-transaction DB session available here).
  Reports remains **NOT locked / NOT complete**.
