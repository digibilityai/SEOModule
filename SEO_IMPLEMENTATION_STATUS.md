...
| **Recommendation Generation — Stage 1 (guarded generation RPC, backend only)** | **`IMPLEMENTED — LOCALLY VERIFIED — ACCEPTED — MODULE-LOCKED (2026-07-24)`.** Additive migration `20260724130000_seo_recommendation_generate.sql` adds two nullable columns (`source_issue_fingerprint`, `generation_method`) and two partial unique indexes (`WHERE is_current`) to `seo_recommendations`, plus one guarded `SECURITY DEFINER` RPC `public.seo_recommendation_generate(p_website_id uuid) RETURNS SETOF seo_recommendations` (`SET search_path=public`; `authenticated` EXECUTE, **anon + PUBLIC revoked up-front**). Converts real, crawler-detected `seo_audit_issues` (status `open`/`in_review` only, from the latest completed audit run) plus 7 fixed on-page templates into canonical `seo_recommendations` rows, reproducing the existing mock's `CATEGORY_TO_AREA` / `ACTION_TYPE_BY_FIX_OWNER` mapping and `ON_PAGE_TEMPLATES` server-side — no AI/LLM, no new categories. Authorizes owner/admin/team_member or global admin (client/anon/non-member/cross-tenant denied, one non-leaking message; missing website == unauthorized). **First real consumer of the existing `is_current`/`superseded_by` versioning**: three-way replace-to-match (insert-new / no-write-if-unchanged / supersede-if-changed-and-still-`suggested`-or-`needs_review` / leave-alone-if-a-human-already-acted / retire-if-resolved-and-untouched) — a human decision (`approved`, `ready_to_publish`, etc.) is never silently overwritten by a later regeneration. Returns the **canonical current recommendation set** (not a transient count).
  ...
  **Implementation state:** migration, RPC, SQL verification suite, rollback script, and this record all exist **only** in a temporary local worktree/branch (`feat/seo-recommendation-generate-stage1`, based on `origin/main` `71ac8fd0fd6087bb5435bea4cca865025bc27967`) — **not committed, not pushed.** No frontend file, no locked module, no crawler table, and no approval-lifecycle code was touched. `Digi_SEO_Test` remains rolled back and untouched; production untouched.

  **Acceptance + lock (2026-07-24):** ~~(1) a genuinely local PostgreSQL/Supabase environment must be made available~~ — **DONE**; ~~(2) genuine local Stage 1 database verification must be performed and pass~~ — **DONE**; ~~(3) Stage 1 must be reviewed and accepted~~ — **DONE, Stage 1 is formally ACCEPTED**; implementation committed to `feat/seo-recommendation-generate-stage1` (based on `origin/main` `71ac8fd`) — **not pushed, not merged to `main`.** **Recommendation Generation Stage 1 (backend only) is formally MODULE-LOCKED** — see `docs/markdown/MODULE_LOCKS.md` (new entry). This lock is narrower than every other locked module in the registry: **no frontend integration, no unit tests, and no authenticated operator/browser acceptance exist for this feature** — Stage 2 (frontend wiring, role gate, unit tests, operator acceptance) remains explicitly deferred and UNLOCKED, its absence is not a defect and is now unblocked for a future, separately-approved task. **No further `Digi_SEO_Test` use is permitted for this feature without an approval explicitly recorded in the controlling ChatGPT instruction trail.** | **LOCKED** (Recommendation Generation Stage 1 backend-only approved scope; Stage 2 frontend remains UNLOCKED) | `supabase/migrations/20260724130000_seo_recommendation_generate.sql`, `supabase/test/seo_recommendation_generate_verification.sql`, `supabase/test/seo_recommendation_generate_rollback_TEST_ONLY.sql`, `SEO_RECOMMENDATION_GENERATION_STAGE1_VERIFICATION.md`, `SEO_RECOMMENDATION_GENERATION_ARCHITECTURE.md`, `SEO_LOCAL_DATABASE_SETUP.md` |
