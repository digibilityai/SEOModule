...
- **Immediate next feature increment:** **Competitor Benchmarking Stage 2B
  (frontend generation integration).** **Stage 2A (guarded generation RPC) is
  DONE — backend-implemented + TEST-verified 2026-07-24** (migration
  `20260724120040_seo_competitor_generate.sql`; `public.seo_competitor_generate(
  uuid) RETURNS integer`; server-derived workspace/actor, advisory lock,
  deterministic heuristic scoring, replace-to-match upsert, truthful
  `data_provenance='estimated'`, anon-revoke folded in; applied in isolation on
  TEST, SSO `20260720121000` still deferred). **Stage 2B remains:** wire the
  Supabase generate path (call the RPC via `runWithServiceAdapter`, no silent
  mock fallback), re-enable Generate/Refresh in Supabase mode for
  owner/admin/team_member only, then run authenticated operator browser
  acceptance. No frontend service/hook/UI change has been made yet.
