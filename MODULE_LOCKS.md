...
- **2026-07-19 — P1a Domain Ownership Verification: real `verify-once` worker-binary run COMPLETE — PASS. P1a is now FORMALLY LOCKED — see the new "P1a — Domain Ownership Verification" entry below.** The last outstanding P1a acceptance item (the real worker binary against `Digi_SEO_Test`) has passed with full evidence (`P1A_DOMAIN_OWNERSHIP_VERIFICATION_SIGNOFF.md` §4 + §10 2026-07-19 entry). This closes the two-item gap noted in the 2026-07-16 entry above. The `verify-once` mode and its files remain the same approved additive extension described in the first bullet of this log; the Crawler 16C–16H lock is unaffected and remains fully in force. P1b is now the next implementation stage (not started). Production untouched.
- **2026-07-19 — P1b Verified-only Crawl Enqueue Enforcement: planning/reference note only (NO unlock, NO scope change, NO approval granted here).** An authoritative P1b implementation plan now exists (`P1B_VERIFIED_ONLY_CRAWL_ENQUEUE_PLAN.md`). P1b will add a verified-ownership precondition inside the **locked** `seo_crawl_request` (the "domain-ownership verification" item listed under this lock's "Deferred scope — remains UNLOCKED"). When implemented, it **must** follow this lock's additive-extension + evidence procedure: additive **new** migration only (`CREATE OR REPLACE FUNCTION public.seo_crawl_request(...)`; the applied `20260713120025` migration is **not** edited); preserve the RPC name/parameter/return contract, grants, role behaviour, idempotency, single-active-job rule, and event creation (only a new verified-ownership rejection is added); `seo_crawl_request_audit` unchanged; the worker untouched; targeted locked-scope regression (16C–16H DB verifications + worker suite) re-run after the six verification scripts receive verified-ownership fixtures. This bullet records the plan for traceability; it does **not** modify any locked scope, does **not** grant implementation approval, and the Crawler 16C–16H lock remains fully in force. A formal **P1b** lock entry will be added only after full P1b acceptance. P1a remains LOCKED and untouched. P1b implementation NOT started. Production untouched.
- **2026-07-19 — P1b implementation-in-progress note (approved additive extension being built; NO lock added; NO applied migration edited; NOT yet executed).** Under explicit approval to perform the narrowly-scoped P1b additive extension, the build artifacts now exist in-repo and are statically reviewed: the new additive migration `supabase/migrations/20260719120034_seo_p1b_verified_only_crawl_enqueue.sql` (`CREATE OR REPLACE FUNCTION public.seo_crawl_request(...)` adding ONLY the verified-ownership guard — `FOR SHARE` on `seo_ownership_verifications` `dns_txt`/`verified`, plain `P0001` message; a byte-for-byte static diff vs the applied Phase 16C body confirms the guard is the only change; name/params/return/SECURITY DEFINER/search_path/grants/checks/precedence/idempotency/single-active-job/INSERT/event unchanged; `seo_crawl_request_audit` and the worker untouched); a TEST-only rollback restoring the exact original; verified-ownership fixtures added to the six 16C–16H verification scripts; a new `supabase/test/seo_p1b_verified_only_crawl_enqueue_verification.sql`; and the companion `P1B_CONCURRENCY_VERIFICATION_GUIDE.md`. **The applied `20260713120025` migration is NOT edited** (a new migration file is added). **No migration has been applied and no verification has been run** — TEST execution awaits explicit approval. The Crawler 16C–16H lock remains fully in force; **P1b is NOT locked** (a formal P1b lock entry follows full acceptance). **P1a remains LOCKED and untouched. Production untouched.** See `P1B_VERIFIED_ONLY_CRAWL_ENQUEUE_PLAN.md` §1; `CURRENT_PROJECT_STATUS.md` (authoritative).
- **2026-07-19 — P1b TEST-applied, verified, and now FORMALLY LOCKED — see the new "P1b — Verified-only Crawl Enqueue Enforcement" entry below.** The additive migration `20260719120034_seo_p1b_verified_only_crawl_enqueue.sql` was applied to `Digi_SEO_Test` (recorded once); the deployed `seo_crawl_request` contract was verified unchanged except the added `FOR SHARE` verified-ownership guard (plain `P0001`; no custom SQLSTATE; `seo_crawl_request_audit` unchanged); the P1b acceptance verification, all six 16C–16H DB regressions (with verified-ownership fixtures), and the worker suite (74/74) **ALL PASS**; the `FOR SHARE` write-time atomicity was proven with a **live two-session** run (revoke-wins rejects; enqueue-wins commits). Fixture residue = 0; production never contacted; rollback not used. This closes the P1b implementation under this lock's additive-extension procedure; the Crawler 16C–16H lock and the P1a lock remain fully in force and unchanged. Evidence: `P1B_VERIFIED_ONLY_CRAWL_ENQUEUE_SIGNOFF.md`; `P1B_CONCURRENCY_VERIFICATION_GUIDE.md`; `CURRENT_PROJECT_STATUS.md` (authoritative).
