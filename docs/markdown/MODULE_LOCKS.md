...
- **2026-09-24 — D-026A: `crawler-worker/Dockerfile` packaging fix (approved proven bug fix, PR #6).** Under explicit human approval, `crawler-worker/Dockerfile` was corrected: the deps stage now runs `npm ci --include=dev` (was `npm ci --omit=dev || npm install --omit=dev`) and the runtime stage's separate, unlocked `npm install --no-save tsx@^4.19.2 typescript@^5.6.0` was removed. Root cause: two independent dependency resolutions (a pruned prod-only tree, then a second unlocked install) instead of one lockfile-driven tree — the earlier `@supabase/supabase-js` `ERR_MODULE_NOT_FOUND` and the later `tsx` `ERR_MODULE_NOT_FOUND` were two symptoms of this same defect, not separate bugs. **Docker-packaging only** — no worker source, mode, verification/crawl logic, Supabase logic, `package.json`/`package-lock.json`/`tsconfig`, or application architecture changed. **Verification:** new image built via Cloud Build under a new immutable tag (`…digi-seo-crawler-worker:edeb5f2b`, commit `edeb5f2be70b70685330e14497f527a394cdae5a`); `--mode=dry-run` and `--mode=verify-once` each run in an isolated Cloud Build sandbox against a deliberately unreachable dummy Supabase URL/key — both started, logged config, reached their real RPC call, and failed cleanly with a network-layer `TypeError: fetch failed` (exit code 1), with **no module-resolution error** for `@supabase/supabase-js` or `tsx`, confirming existing mode dispatch is intact. `seo-test-verify-once-scheduler` remained **PAUSED** throughout and the live `seo-test-verify-once` Cloud Run Job image was **not** changed (still `…001564f7`); no real Supabase/DNS/production data touched. **Amends the existing PR #6 head** (was `33bc01ed`, now `edeb5f2b`'s source commit) — requires independent re-review before merge per the locked-module procedure.
- **2026-09-24 — D-026A: TEST worker-runtime real DNS acceptance, `seo-test.digibility.ai` — PASS (operator-approved acceptance run, no code/config change).** With founder-controlled DNS on `seo-test.digibility.ai`, a real end-to-end acceptance of the deployed `edeb5f2b` image was completed on the live `seo-test-verify-once` Cloud Run Job (closing the gap the July 19 acceptance evidence left open: that run proved the verification/DNS/RPC code path locally, and the `edeb5f2b` Cloud Build sandbox runs proved packaging only, but neither had exercised a real DNS TXT resolution from inside the deployed Cloud Run Job's own network/runtime). TXT record `_digibility-site-verification.seo-test.digibility.ai` confirmed publicly resolvable (authoritative-nameserver + public-resolver lookups) before any execution. Manual single execution `seo-test-verify-once-fdj92` (2026-09-24T18:48:22Z–18:48:35Z): worker started against real `Digi_SEO_Test` (`snyzotgwwfomgafrsvfm`), claimed verification `a2e8b22c-c826-4def-8fce-5caa4ae6cca4` (website `33cc1da7-f6d1-4b0e-90e7-9de9e59f4c17`), logged `outcome=verified`, `succeededCount=1`, exit code 0 — the first real `verified` outcome (July 19's real run legitimately resolved `failed`/`dns_not_found`). `seo-test-verify-once-scheduler` was then resumed to its existing `ENABLED` state on its existing `*/5 * * * *` schedule and existing Cloud Run Job target — no other scheduler/job/secret/IAM/migration/source change. This is a **runtime acceptance record only**; no worker source, verification logic, migration, or crawl contract was touched, and no new lock is added beyond the existing P1a lock (2026-07-19).
- **2026-09-25: D-026A application-linking completion, IMPLEMENTED, NOT yet applied/merged (status note, no lock added).** Closes the gap left after the SEO PR-1 linking foundation (PR #3): identity could be established for cases A, B and D, but nothing resolved a website or spent the redeemed intent on `seo_brain_link_authorize`, so a case D customer in particular could see a false "connected" state after nothing more than an identity mapping. Adds exactly two additive RPCs in `20260925120000_seo_brain_link_completion.sql` (`seo_brain_link_intent_continue_by_code`, service_role only, mints a case D magic-link sign-in for an already-mapped user without creating anything; `seo_brain_resolve_link_website`, authenticated only, resolves or, only when nothing matches, creates the single website for the intent's host, with no customer selector, then reuses the existing unchanged `seo_ownership_verification_initiate`) and one new Edge Function route (`link-intent/continue`, not machine-secret gated, same trust shape as `provision`). `seo_brain_link_authorize` itself is unchanged. `SeoBrainConnectPage.tsx` now drives every case (A, B, D) through resolution and authorization before ever showing a connected state, refuses a case D live-session mismatch client-side (`decideCaseDSessionAction`) rather than silently switching identity, and shows a pending-verification screen (not "connected") when DNS ownership is still outstanding. Full detail: `SEO_BRAIN_MODULE_INTERFACE.md` section 9.7. **Verification:** `npm test` 457/457 pass (43 new, covering case D continuation success/refusal, the session-mismatch decision, website resolution/authorization outcome typing); root `tsc --noEmit` and `npm run build` both clean. **The migration itself is NOT applied to any Supabase project** (no local Postgres/Docker/`SUPABASE_SERVICE_ROLE_KEY` available in this environment, consistent with prior D-026A entries above) and its two new RPCs are therefore structurally reviewed, not live-tested against a real database; that run, plus authenticated browser click-through of all four cases, is the remaining operator-acceptance item before any lock is considered. No crawl, DNS worker, scheduler, or existing RPC/table/policy was touched.
- **2026-09-25 (correction) — D-026A: Case D continuation narrowed to short-lived + single-use, independent-review blocker, STILL NOT applied/merged.** Independent review of PR #7 found `seo_brain_link_intent_continue_by_code` (same file, still unapplied) let bare possession of a case D launch code mint a fresh authenticated session repeatedly across the whole ~30 minute `consent_expires_at` window, instead of once, immediately after redeem. Corrected in place in `20260925120000_seo_brain_link_completion.sql` (the migration is still unapplied, so amending it directly kept one coherent additive migration rather than a second one on top): eligibility now reuses `redemption_expires_at` (the same short ~120 second, issuance-time primitive `seo_brain_link_intent_pending_by_code` already uses for case A) instead of `consent_expires_at`, and the function is now a single atomic `UPDATE ... WHERE continuation_consumed_at IS NULL ... RETURNING` against one new additive column (`continuation_consumed_at`, nullable timestamptz), so a continuation can succeed at most once and two concurrent attempts for the same code cannot both win. `consent_expires_at`, `status`, `redemption_outcome`, `seo_brain_resolve_link_website` and `seo_brain_link_authorize` are all untouched. New focused SQL verification: `supabase/test/seo_brain_link_intent_continue_verification.sql` (NOT run as part of this PR, same convention as the sibling `seo_brain_link_intents_verification.sql`; structurally reviewed only, for the same no-local-Postgres reason as the original entry above), proving single continuation success, a second attempt with the same code failing, an elapsed-short-window continuation failing while `consent_expires_at` remains valid and untouched, case A unaffected, and case D's redeem→continue completion still leaving the row eligible for `authorize`. `npm test` (unchanged suite, no JS/TS source touched by this correction), root `tsc --noEmit` and `npm run build` re-run clean. `SEO_BRAIN_MODULE_INTERFACE.md` section 9.7 updated to describe the corrected semantics in place of the superseded "no new column needed" reasoning.
- **2026-09-25 (release) — D-026A: application-linking completion MERGED to `main` (PR #7, no lock added, status note only).** Following the idempotent repeat-connect fix (`c33527c`) and a founder-performed live retry on TEST confirming the prior "already connected" defect no longer reproduces, PR #7 was merged to `main` as merge commit `edd60f5bacc1ee43aef4e725c3ce0c2c8eafc870` (two-parent merge, base `d04ac414`, head `c33527c611065c00af4bc9316baa19cfc6fdb6fa` — same merge-commit convention as PR #2–#6). Independently verified in the merging session: PR head matched the accepted SHA exactly (`--match-head-commit`), no commits landed after that head, `main` had no branch protection / no required checks configured, the merged `main` tree hash is byte-identical to the accepted head's tree, local `main` fast-forwarded cleanly to `origin/main`, and post-merge `tsc --noEmit`, `npm test` (458/458 pass), and `npm run build` all ran clean on the merged tree. TEST deployment state (`seo-module-api` v6, TEST frontend revision `digi-seo-frontend-test-00009-gw6`, migrations `20260925120000/130000/140000` applied) and the founder acceptance walkthrough are carried forward as previously reported/operator-accepted, not re-verified in this session — no migrations were reapplied and TEST was not redeployed, since the merged tree is content-equivalent to what was already deployed and accepted. Production untouched. This entry closes out the sequence of status notes above (2026-09-25 and 2026-09-25 correction) for tracking purposes only; whether this rises to a formal module lock is a separate decision not made here.
